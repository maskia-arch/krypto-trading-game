...
module.exports = (db) => ({
  async getLeaderboard(filter = 'profit_season', limit = 20) {
    ...
    // v0.3.21: Filter out users with zero activity (never traded)
    leaders = leaders.filter(l => {
      const hasVolume = Number(l.total_volume) > 0;
      const hasAssets = !!userAssets[l.id] && userAssets[l.id].length > 0;
      const hasPositions = !!userLev[l.id] && userLev[l.id].length > 0;
      return hasVolume || hasAssets || hasPositions;
    });
