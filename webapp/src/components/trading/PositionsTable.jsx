...
export default function PositionsTable() {
  const { leveragePositions, prices, closeLeveragePosition, showToast } = useStore();
  const [closingId, setClosingId] = useState(null);

  // Keine Trades vorhanden: Kompakte Anzeige
  ...
  return (
    <div className="space-y-4 tab-enter">
      ...
      {leveragePositions.map(pos => {
        // Konvertierung für Echtzeit-PnL basierend auf Store-Preisen
        ...
        return (
          <div key={pos.id} className="card p-3 border border-white/5 relative overflow-hidden bg-gradient-to-br from-[#0a0c14] to-black/80 backdrop-blur-xl shadow-lg transition-all duration-300">
            ...
            <button
              ...
              disabled={!!closingId}
              className="w-full relative z-10 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all border border-white/10 bg-white/5 text-white active:scale-[0.98] disabled:opacity-50 overflow-hidden group"
            >
              {closingId === pos.id ? (
